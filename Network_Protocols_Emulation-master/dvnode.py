...
def broadcast_to_neighbor():
    ...
    for neighbor_port in neighbor_table['neighbors']:
        s.sendto(bytes(json.dumps(routing_table), 'utf-8'), ('', neighbor_port))
    ...
    for destination in client_packet['destination']:
        if destination != routing_table['local_port']:
            ...
            if destination not in routing_table['destination']:
                ...
                routing_table['distance'].append(
                    neighbor_table['distance'][neighbor_index] + client_packet['distance'][destination_index])
            else:
                routing_table_index = routing_table['destination'].index(destination)
                if (neighbor_table['distance'][neighbor_index] + client_packet['distance'][destination_index]) < \
                        routing_table['distance'][routing_table_index]:
                    routing_table['distance'][routing_table_index] = neighbor_table['distance'][neighbor_index] + \
                                                                     client_packet['distance'][destination_index]
...
def dvnode():
    while True:
        try:
            ...
            client_packet = json.loads(client_packet.decode('utf-8'))
        except socket.error:
            continue
        if not info_table['setup']:
            info_table['setup'] = True
            broadcast_to_neighbor()
        if update_routing_table(client_packet):
            info_table['start'] = True
            ...
            for i in range(len(routing_table['destination'])):
                if routing_table['nexthop'][i]:
                    print('- (' + str(routing_table['distance'][i]) + ') -> Node ' + str(
                        routing_table['destination'][i]) + '; Next hop -> Node ' + str(routing_table['nexthop'][i]))
                else:
                    print(
                        '- (' + str(routing_table['distance'][i]) + ') -> Node ' + str(routing_table['destination'][i]))
        if info_table['start']:
...
if __name__ == '__main__':
    ...
    try:
        ...
        routing_table = {'local_port': None, 'destination': [], 'nexthop': [], 'distance': [],
                         'timestamp': None}  ## if direct then nexthop is None
        init()
