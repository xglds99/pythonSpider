...
def broadcast_to_neighbor():
	...
	for neighbor_port in neighbor_table['neighbors']:
		s.sendto(bytes(json.dumps(routing_table), 'utf-8'),('127.0.0.1',neighbor_port))
		...
		if update_routing_table(client_packet) == True:
			...
			for i in range(len(routing_table['destination'])):
				if routing_table['nexthop'][i]:
					print('- ('+str(format(routing_table['distance'][i],'.1f'))+') -> Node '+str(routing_table['destination'][i])+'; Next hop -> Node '+str(routing_table['nexthop'][i]))
				else:
					print('- ('+str(format(routing_table['distance'][i],'.1f'))+') -> Node '+str(routing_table['destination'][i]))
		if info_table['start']:
...
if __name__ == '__main__':
	...
	try:
		...
		s.bind(('127.0.0.1',routing_table['local_port']))
		if info_table['start']:
			broadcast_to_neighbor()
			info_table['start'] = False
			info_table['setup'] = True
