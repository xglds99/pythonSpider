...
def send_packet(packet, peer_port):
    s.sendto(bytes(json.dumps(packet), 'utf-8'), ('127.0.0.1', peer_port))
    if packet['ack'] == False:
...
def discard_packet(received_number_list, packet, drop_mode, drop_value, n):
    ### return boolean value, True: discard, False: not_discard
    if drop_mode == '-d':
        print("received_number_list[0]:" +str(received_number_list[0]))
        print("drop_value:" + str(drop_value))
        if len(str((received_number_list[0]) % float(drop_value))) >= 19:
            return False
        else:
            if packet['ack'] == False:
                print(
...
def receive(peer_port, timer_list, window_list, data_buffer, received_seq_list, drop_mode, drop_value):
    ...
    while True:
        ...
        if packet['ack'] == False and packet['seq'] == -2:
            ...
            if discard_packet(received_number_list, packet, drop_mode, drop_value, n) == False:
                if packet['ack'] == False and packet['seq'] != -2:
                    ### receiver
...
if __name__ == '__main__':
    try:
        ...
        s.bind(('127.0.0.1', self_port))
        t1 = Thread(target=send,
                    args=(peer_port, window_size, drop_mode, drop_value, ack_list, data_buffer, timer, window_list))
        t2 = Thread(target=receive,
                    args=(peer_port, timer, window_list, data_buffer, received_seq_list, drop_mode, drop_value))
        t1.setDaemon(True)
        t2.setDaemon(True)
        t1.start()
        t2.start()
