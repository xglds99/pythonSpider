            ...
            if update_table['dv_update'] == True:
                ...
                for port, value in routing_table.items():
                    if isinstance(value, dict):
                        if value['nexthop']:
                            print(
                                '- (' + str(format(value['distance'],'.1f')) + ') -> Node ' + str(port) + '; Next hop -> Node ' + str(
                                    value['nexthop']))
                            sys.stdout.flush()
                        else:
                            print('- (' + str(format(value['distance'],'.1f')) + ') -> Node ' + str(port))
                            sys.stdout.flush()
    ...
    while True:
        ...
        if DV_timer.timeout():
            ...
            for port, value in routing_table.items():
                if isinstance(value, dict):
                    if value['nexthop']:
                        print('- (' + str(format(value['distance'],'.1f')) + ') -> Node ' + str(port) + '; Next hop -> Node ' + str(
                            value['nexthop']))
                        sys.stdout.flush()
                    else:
                        print('- (' + str(format(value['distance'],'.1f')) + ') -> Node ' + str(port))
                    ...
                    if value['receive_from'] or value['send_to']:  ## if it is neighor
                        if original_distance[port] != value['distance']:  ## if update, update itself is not here!!
                            ...
                            s.sendto(bytes(json.dumps(routing_table), 'utf-8'), ('127.0.0.1', int(port)))
            DV_timer.start()
...
if __name__ == '__main__':
    info_table = {'setup': False, 'message_length': 10,
                  'is_last': False}  ### port:expected_number!!!!  ## last -> start == True, alredy setup -> start == True, setup: send when first received
    routing_table = {'tag': 'routing_table', 'local_port': None,
                     'timestamp': None}  # 'port':{'receive_from':False,'send_to':False,'distance':100,'received_number':0'discarded_number':0,'nexthop':None,'probability':None}
            ...
            if value['send_to'] == True:
                ...
                if info_table['is_last'] == False:
                    ...
                    for port, value in routing_table.items():
                        if isinstance(value, dict):
                            if value['nexthop']:
                                print('- (' + str(format(value['distance'],'.1f')) + ') -> Node ' + str(
